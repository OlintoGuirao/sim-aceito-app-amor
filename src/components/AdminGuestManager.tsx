...
const AdminGuestManager: React.FC = () => {
  const [guests, setGuests] = useState<Guest[]>([
    { id: '1', name: 'Ana Silva', email: 'ana@email.com', status: 'confirmed', group: 'Família', table: 1 },
    { id: '2', name: 'Carlos Santos', email: 'carlos@email.com', status: 'pending', group: 'Amigos' },
    { id: '3', name: 'Mariana Costa', email: 'mariana@email.com', status: 'declined', group: 'Trabalho' },
    { id: '4', name: 'Pedro Oliveira', email: 'pedro@email.com', status: 'pending', group: 'Família' },
  ]);
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      default: return 'bg-[#f5e6d3] text-[#5f161c]';
    }
  };
        ...
        <TabsContent value="add">
          <Card className="p-6 bg-white">
            ...
            <div className="space-y-4">
              ...
              />
              <Input
                placeholder="Grupo (ex: Família, Amigos, Trabalho)"
                value={newGuestGroup}
        ...
        <TabsContent value="list">
          <Card className="p-6 bg-white">
            ...
            <div className="space-y-3">
              {guests.map((guest) => (
                <div key={guest.id} className="flex items-center justify-between p-3 bg-[#f5e6d3]/10 rounded-lg">
                  <div>
                    <div className="font-medium text-[#5f161c]">{guest.name}</div>
                    <div className="text-sm text-[#5f161c]/80">{guest.email}</div>
                    {guest.group && (
                      <div className="text-xs text-[#5f161c]/60">Grupo: {guest.group}</div>
                    )}
                    {guest.table && (
                      <div className="text-xs text-[#5f161c]/60">Mesa: {guest.table}</div>
                    )}
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <Button 
                      size="sm" 
                      variant="outline"
                      className="border-[#5f161c]/20 hover:bg-[#5f161c] hover:text-white"
                    >
                      Editar
                    </Button>
                  </div>
        ...
        <TabsContent value="qrcodes">
          <Card className="p-6 bg-white">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {guests.map((guest) => (
                <Card key={guest.id} className="p-4 text-center bg-[#f5e6d3]/10">
